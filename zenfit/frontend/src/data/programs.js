 ...
 *
 * `image` points at a file under `public/Programs/` (capital P — matches the
 * folder as it actually exists on disk; a plain static path, not an ES
 * import, so a not-yet-delivered photo 404s at runtime instead of failing the
 * build) — ProgramCard in WorkoutsScreen.jsx falls back to a gradient tile on
 * load error, so a missing file is safe, it just won't show a photo.
 */
