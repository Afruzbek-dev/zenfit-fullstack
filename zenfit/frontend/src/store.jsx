...
export function AppProvider({ children }) {
   ...
   * `burned` and its counters; `remaining` is left untouched on purpose.
   */
  const logWorkout = useCallback(async (payload) => {
    ...
    // what was sent — otherwise the display drifts from the day's real total.
    const kcal = res.workoutLog?.kcal || 0;
    setWorkoutHistory((prev) => [res.workoutLog, ...prev]);
    setSummary((prev) =>
      prev ? { ...prev, burned: prev.burned + kcal, workoutCount: prev.workoutCount + 1 } : prev
    );
